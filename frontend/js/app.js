...
const createCard = (product) => {
  return `<div class="cell m-2">
            <div class="box has-background-white has-text-centered">
              <p class="title is-6 has-text-black">${product.title.substring(
                0,
                15
              )}</p>
              ...
              <button class="button is-large is-primary is-rounded">Comprar</button>
            </div>
          </div>`;
};
...
const preLoadProducts = () => {
  ...
  const womensclothingProducts = document.querySelector(
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  validationInput(
    'inputName',
    'adviceName',
    'nombre no válido, es demasiado corto'
  );
  validationInput(
    'inputEmail',
    'adviceEmail',
    'correo electrónico no válido, por favor compruébelo'
  );
  validationInput(
    'inputPhone',
    'advicePhone',
    'teléfono no válido, por favor compruébelo'
  );
  validationInput(
    'inputMessage',
    'adviceMessage',
    'mensaje no válido, es demasiado corto',
    'textarea'
  );
